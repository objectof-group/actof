...
public class EditorUtils {
	...
	public static boolean isStereotypeSupported(Stereotype st) {
        switch (st) {
			...
			case COMPOSED: //editing supported through child nodes, not directly
			case FN:
			case MEDIA:
			case SET:
				return false;
	...
	public static Editor createConfiguredEditor(Cell<CompositeEntry> cell, final StringConverter<CompositeEntry> converter, Stereotype st) {
		 
		 CompositeEntry entry = cell.getItem();
		 Editor editor = createEditor(entry, st);
	...
	public static Editor createEditor(CompositeEntry entry, Stereotype st) {
