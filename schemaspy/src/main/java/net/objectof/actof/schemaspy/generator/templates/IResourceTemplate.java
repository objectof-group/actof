...
import net.objectof.model.impl.facets.IResource;
import net.objectof.rt.impl.base.Util;


public class IResourceTemplate extends ITemplate<IKind<?>>implements Facet<IKind<?>> {
    ...
    protected Writer defineWriter(IKind<?> aKind) throws IOException {
        ...
        dir.mkdirs();
        String fileName = 'I' + Util.initCap(Util.selector(aKind.getComponentName())) + ".java";
        File f = new File(dir, fileName);
        return new FileWriter(f);
    }

    protected String getImplKind(IKind<?> aResource) {
        Object kind = aResource.getProperty("objectof.net:1401/model/implementationKind");
        if (kind != null) { return kind.toString(); }
        return IKind.class.getName() + '<' + Composite.class.getName() + '>';
    }

    protected String getSuperClass(IKind<?> aResource) {
        Object base = aResource.getProperty("objectof.net:1401/model/implementation");
